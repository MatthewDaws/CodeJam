...
def check_assignments(edges, assignment, index):
    ...
    Returns a list of new indicies which are added to assignment.
    Throws InvalidAssignment if problem."""
    otherside = { "L":"R", "R":"L" }
    nhbs = ( j for (i,j) in edges if i == index )
    changed = []
    for nhb in nhbs:
...
def all_assignments(side_road, cars):
    ...
    for index, car in enumerate(cars):
        ...
        edges.extend( (index, j) for j in will_overtake )
        edges.extend( (j, index) for j in will_overtake )

    # Add in those which have no interaction
    for index, car in enumerate(cars):
        if index not in initial and not any( index == i for (i,_) in edges ):
            #initial[index] = side_road[index]
            initial[index] = "L"
